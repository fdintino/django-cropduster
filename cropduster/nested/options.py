...
class InlineModelAdmin(BaseModelAdminMixin, _InlineModelAdmin):
    ...
    def __init__(self, parent_model, admin_site):
        self.inlines = []
        fields = self.model._meta.get_m2m_with_model()
        for field, m in fields:
            if hasattr(field, 'rel'):
                ...
                from .formsets import CropDusterNestedInlineFormSet
                if issubclass(rel_model, Image):
                    inline_template = getattr(self, 'nested_inline_template', None)
                    NestedInlineFormSet = cropduster_inline_factory(
                        field.sizes, field.auto_sizes, field.default_thumb,
                        model=rel_model,
                        formset=CropDusterNestedInlineFormSet,
                        template=inline_template)
                    # NestedInlineFormSet = nested_inline_formset_factory(InlineFormSet)
                    self.inlines.append(NestedInlineFormSet)
